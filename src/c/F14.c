...
void daftarCheckUp(ListDinUser *UserData, Config *rumahsakit, int loginId){
    ...
    for(int baris = 0; baris < rumahsakit->denah.rows; baris++){
        for(int kolom = 0; kolom < rumahsakit->denah.cols; kolom++){
            ...
            const char* nama = cari_user(*UserData, rumahsakit->denah.contents[baris][kolom].dokterID);   
    ...
    printf("Anda terdaftar pada antrian %s di ruangan %s.\n", cari_user(*UserData, rumahsakit->denah.contents[choice_holder[choice][0]][choice_holder[choice][1]].dokterID), rumahsakit->denah.contents[choice_holder[choice][0]][choice_holder[choice][1]].kodeRuangan);
